...
const Stores = () => {
    ...
    const loadMerchants = async () => {
        ...
        try {
            ...
            setError("Gagal memuat merchant. Silakan coba lagi.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            console.error(err);
            alert("Gagal membuat merchant: " + (err.response?.data?.message || err.message));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("Apakah Anda yakin ingin menonaktifkan merchant ini? Ini akan membatalkan langganan mereka.")) return;
        try {
            ...
            <div className="space-y-6 max-w-7xl mx-auto overflow-x-hidden">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Kelola toko dan penyewa yang terdaftar</p>
                    </div>
                    <button
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border-none rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-slate-900 dark:text-white"
                            value={searchTerm}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
                        ...
                        <h3 className="text-lg font-medium text-slate-900 dark:text-white">Tidak Ada Merchant Ditemukan</h3>
                        <p className="text-slate-500">Mulai dengan menambahkan merchant baru.</p>
                    </div>
                ) : (
                            ...
                            <div key={merchant.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow p-6 relative group">
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 dark:text-slate-400">{merchant.tenant?.name || 'Tenant Tidak Diketahui'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center text-sm text-slate-600 dark:text-slate-400">
                                        ...
                                        <span>{merchant.users?.[0]?.email || 'Tidak Ada Email'}</span>
                                    </div>
                                    <div className="flex items-center text-sm text-slate-600 dark:text-slate-400">
                                        <MapPin size={16} className="mr-2" />
                                        <span className="line-clamp-1">{merchant.location || 'Tidak Ada Lokasi'}</span>
                                    </div>
                                    <div className="flex items-center text-sm text-slate-600 dark:text-slate-400">
                                        <Calendar size={16} className="mr-2" />
                                        <span>Bergabung {new Date(merchant.createdAt).toLocaleDateString()}</span>
                                    </div>
                                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-xl border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
                            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-900 z-10">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Tambah Merchant Baru</h2>
                                <button
                            ...
                            <form onSubmit={handleCreate} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Bisnis</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Alamat / Lokasi</label>
                                    <textarea
                                ...
                                <div className="pt-4 flex space-x-3">
                                    ...
                                    >
                                        {submitting ? <Loader className="animate-spin" size={20} /> : 'Buat Merchant'}
                                    </button>
                                </div>
                            </form>
                        </div>
