...
const RealtimeBadge = () => {
  ...
  return (
    <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs ${state.connected ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
      ...
      <span className="text-slate-500 dark:text-slate-400">Last Sync: {formatTime(state.lastSync)}</span>
    </div>
  )
}
