...
const getStoreChatUser = async (req, res) => {
    try {
        const { id } = req.params;
        const store = await prisma.store.findUnique({
            where: { id },
        ...
        // Find OWNER or ADMIN for this tenant
        ...
        if (!user) return errorResponse(res, "Chat user not found for this store", 404);
        return successResponse(res, user);
    } catch (error) {
        return errorResponse(res, "Failed to fetch store chat user", 500, error);
    }
};
...
const getProduct = async (req, res) => {
    try {
        ...
        return successResponse(res, { ...product, category: product.store.category });
    } catch (error) {
        return errorResponse(res, "Fetch product failed", 500, error);
    }
};
...
const getRecommendations = async (req, res) => {
    try {
        ...
        // Base fetch for popular items fallback
        const fetchPopular = async () => {
            return await prisma.product.findMany({
                ...
                select: {
                    id: true, name: true, sellingPrice: true, imageUrl: true,
                    averageRating: true, reviewCount: true, storeId: true,
                    store: { select: { name: true, category: true, latitude: true, longitude: true } }
                }
            });
        };

        if (!phone) {
            recommendedProducts = await fetchPopular();
        } else {
            // Find past completed transactions
            const pastOrders = await prisma.transaction.findMany({
                where: { 
                    customerPhone: phone, 
                    source: 'MARKET',
            ...
            if (pastOrders.length === 0) {
                ...
                const items = await prisma.transactionItem.findMany({
                    where: { transactionId: { in: pastOrders.map(o => o.id) } },
                ...
                const catCounts = {};
                items.forEach(item => {
                    const cat = item.product?.store?.category;
                    if (cat) {
                ...
                if (topCat) {
                    recommendedProducts = await prisma.product.findMany({
                        where: { 
                            isActive: true, 
                            store: { isActive: true, category: topCat } 
                        },
                        take: 15,
        ...
        // Map distances if lat/long are provided
        if (lat && long) {
            const userLat = parseFloat(lat);
            const userLong = parseFloat(long);
            recommendedProducts = recommendedProducts.map(p => {
                ...
                if (p.store.latitude && p.store.longitude) {
                    const R = 6371; 
                    const dLat = (p.store.latitude - userLat) * Math.PI / 180;
                    const dLon = (p.store.longitude - userLong) * Math.PI / 180;
                    const a = Math.sin(dLat/2) * Math.sin(dLat/2) +
                            Math.cos(userLat * Math.PI / 180) * Math.cos(p.store.latitude * Math.PI / 180) * 
                            Math.sin(dLon/2) * Math.sin(dLon/2);
                    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
                    dist = R * c;
                }
            ...
            // Sort by distance if they match the rating criteria
        } else {
            recommendedProducts = recommendedProducts.map(p => ({ ...p, category: p.store.category }));
        }

        return successResponse(res, recommendedProducts);
    } catch (error) {
        return errorResponse(res, "Failed to get recommendations", 500, error);
    }
};
