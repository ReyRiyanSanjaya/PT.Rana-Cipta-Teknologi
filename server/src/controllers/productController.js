...
const getProducts = async (req, res) => {
    try {
        ...
        const products = await prisma.product.findMany({
            ...
            include: { 
                category: true,
                stocks: storeId ? { where: { storeId } } : false
            },
            orderBy: { name: 'asc' }
        });

        // Map to use store specific stock if available
        const mappedProducts = products.map(p => {
            let finalStock = p.stock;
            if (storeId && p.stocks && p.stocks.length > 0) {
                finalStock = p.stocks[0].quantity;
            }
            // Remove stocks array from response to keep it clean
            const { stocks, ...rest } = p;
            return { ...rest, stock: finalStock };
        });

        return successResponse(res, mappedProducts, "Products retrieved successfully");
    } catch (error) {
