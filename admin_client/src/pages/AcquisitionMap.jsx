...
const AcquisitionMap = () => {
    ...
    useEffect(() => {
        ...
        fetchStores();
        setFocusBounds(null);
    }, [activeCity, statusFilter, planFilter, dateFrom, dateTo, scope]);
    ...
    useEffect(() => {
        ...
        if (!baseDistricts || baseDistricts.length === 0) {
            setDistrictStats([]);
            return;
        }
        const stats = baseDistricts.map(district => {
    ...
    }, [activeCity, stores, nationalDistricts, scope]);
    ...
    const mapCenter = scope === 'CITY' ? activeCity.center : INDONESIA_VIEW.center;
    const mapZoom = scope === 'CITY' ? activeCity.zoom : INDONESIA_VIEW.zoom;
    const scopeLabel = scope === 'CITY' ? activeCity.name : INDONESIA_VIEW.name;

    return (
        <div className="relative overflow-x-hidden w-full space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="bg-white/80 backdrop-blur-md p-1.5 rounded-xl border border-white/50 shadow-sm flex items-center space-x-1 flex-wrap">
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 flex items-center ${scope === 'CITY'
                                    ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-100'
                                    : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 flex items-center ${scope === 'INDONESIA'
                                    ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-100'
                                    : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
                                }`}
                        >
                    ...
                    <>
                        ...
                        <div className="h-[650px] w-full">
                            <MapContainer
                                ...
                                zoomControl={false}
                            >
                                <MapController center={mapCenter} zoom={mapZoom} bounds={focusBounds} />
