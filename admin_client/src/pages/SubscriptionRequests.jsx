...
export default function SubscriptionRequests() {
    ...
    const filteredRequests = requests.filter(req => {
        const matchesSearch = 
            ...
            (req.selectedBank?.bankName?.toLowerCase() || '').includes(search.toLowerCase());
    ...
    const tabs = [
        { id: 'ALL', label: 'All Requests', icon: Filter, color: 'text-slate-600', activeColor: 'bg-slate-50 text-slate-700 border-slate-200' },
                                            ...
                                            {req.selectedBank ? (
                                                <div className="flex flex-col">
                                                    <span className="font-medium text-slate-900">
                                                        {req.selectedBank.bankName || 'Bank'}
                                                    </span>
                                                    {(req.selectedBank.accountNumber || req.selectedBank.accountName) && (
                                                        <span className="text-xs text-slate-500 mt-1">
                                                            {(req.selectedBank.accountNumber || '')}
                                                            {req.selectedBank.accountNumber && req.selectedBank.accountName ? ' • ' : ''}
                                                            {(req.selectedBank.accountName || '')}
                                                        </span>
                                                    )}
                                                    {req.selectedBank.label && (
