...
const MerchantDetail = () => {
    ...
    const fetchDetail = async () => {
        try {
            ...
            if (!raw) throw new Error('No merchant data');
            const data = { ...raw };

            const resolvedStoreId = data.storeId || data.id || data.store?.id;
            if (resolvedStoreId) data.storeId = resolvedStoreId;
            if (!data.tenantId && data.tenant?.id) data.tenantId = data.tenant.id;
    ...
    const fetchTransactions = async () => {
        try {
            ...
            const list = Array.isArray(container.transactions) ? container.transactions : [];
            const filtered = list.filter((t) => {
                const storeIdFromTxn = t.storeId || t.store?.id;
                return storeIdFromTxn ? String(storeIdFromTxn) === String(merchant.storeId) : false;
            });
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div className="mb-8">
                <Button variant="outline" onClick={() => navigate('/admin/merchants')} className="mb-4" icon={ArrowLeft}>
                    Back to Merchants
                </Button>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        ...
                        <Button variant="outline" onClick={() => navigate(`/admin/merchants/${id}/menu`)} icon={List}>Atur Produk</Button>
