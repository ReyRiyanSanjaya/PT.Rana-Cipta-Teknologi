    ...
    const handleBannerSubmit = async () => {
        try {
            ...
            const payload = {
                ...
                actionType: bannerForm.actionType,
                actionTarget: bannerForm.actionTarget,
            };

            if (selectedItem) {
                await api.put(`/wholesale/banners/${selectedItem.id}`, payload);
            } else {
                await api.post('/wholesale/banners', payload);
            }
            closeModal();
            fetchBanners();
        } catch (error) {
                        ...
                        <div className="space-y-4">
                            ...
                            {modalType === 'banner' && (
                                <>
                                    ...
                                    <Input label="Description" value={bannerForm.description} onChange={e => setBannerForm({...bannerForm, description: e.target.value})} />
                                    <div>
                                        ...
                                        >
                                            ...
                                            <option value="INTERNAL_ROUTE">Internal Page</option>
                                            <option value="EXTERNAL_URL">External Link</option>
                                        </select>
                                    </div>
                                    {bannerForm.actionType !== 'NONE' && (
                                        <Input
                                            label={bannerForm.actionType === 'EXTERNAL_URL' ? 'External URL (https://...)' : 'Internal Route (contoh: /kulakan, /promo)'}
                                            value={bannerForm.actionTarget}
                                            onChange={e => setBannerForm({...bannerForm, actionTarget: e.target.value})}
                                        />
                                    )}
                                </>
                            )}
