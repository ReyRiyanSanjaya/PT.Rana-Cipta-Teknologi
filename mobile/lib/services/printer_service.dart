...
class PrinterService {
  ...
  Future<void> printReceipt(Map<String, dynamic> transaction, List<Map<String, dynamic>> items, {String? storeName, String? storeAddress}) async {
    ...
    final paperSize = prefs.getString('receipt_paper_size') ?? MerchantConfig.defaultPrinterPaperSize;
    ...
    try {
      bluetooth.isConnected.then((isConnected) {
        if (isConnected == true) {
          ...
          bluetooth.printCustom(MerchantConfig.receiptFooterText, 0, 1);
  ...
  // -- 6. PRINT BARCODE IMAGE --
  Future<bool> printBarcodeImage(Uint8List pngBytes, {String? title}) async {
    try {
      if (!_isConnected) {
        await autoConnect();
        if (!_isConnected) {
      ...
      if (title != null && title.trim().isNotEmpty) {
        bluetooth.printCustom(title.trim(), 1, 1);
      }
      // The plugin supports printing image bytes
      ...
      debugPrint("Print Barcode Error: $e");
      return false;
    }
  }
  ...
  Future<void> _ensureBluetoothIsOn() async {
    try {
      ...
      debugPrint("Failed to turn on Bluetooth automatically: $e");
    }
  }
