...
class _PromoHubScreenState extends State<PromoHubScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final titleStyle =
        GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 16, color: colorScheme.onSurface);
      ...
      body: ListView(
        ...
        children: [
          ...
          _PromoQuickActions(
            ...
            onOpenCrossSell: () {
              _openCrossSellSheet(context);
            },
            onOpenUpsell: () {
              _openUpsellSheet(context);
            },
          ),
  ...
  Future<void> _openCrossSellSheet(BuildContext context) async {
    await showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      showDragHandle: true,
      builder: (_) => _CrossSellSheet(
        onShare: _shareText,
        onWhatsApp: _shareViaWhatsApp,
      ),
    );
  ...
  Future<void> _openUpsellSheet(BuildContext context) async {
    await showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      showDragHandle: true,
      builder: (_) => _UpsellSheet(
        onShare: _shareText,
        onWhatsApp: _shareViaWhatsApp,
      ),
    );
...
class _PromoQuickActions extends StatelessWidget {
  ...
  final VoidCallback onOpenCrossSell;
...
class _CrossSellSheet extends StatefulWidget {
  final Future<void> Function(String text) onShare;
  final Future<void> Function(String text) onWhatsApp;
  const _CrossSellSheet({required this.onShare, required this.onWhatsApp});
  @override
  State<_CrossSellSheet> createState() => _CrossSellSheetState();
}

class _CrossSellSheetState extends State<_CrossSellSheet> {
  late final Future<List<Map<String, dynamic>>> _pairsFuture;
  late final Future<List<Map<String, dynamic>>> _bundleFuture;
  ...
  void initState() {
    ...
    final end = DateTime.now();
    final start = end.subtract(const Duration(days: 30));
    _pairsFuture = DatabaseHelper.instance.getFrequentlyBoughtTogether(
        start: start, end: end, limit: 12);
    _bundleFuture =
        AiService().computeBundleSuggestions(start: start, end: end, limit: 50);
  }

  String _formatBundleCaption({
    required String nameA,
    required String nameB,
    required double originalComboPrice,
    required double bundlePrice,
  }) {
    return AiService().generateBundleCaption(
        nameA: nameA,
        nameB: nameB,
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding:
          EdgeInsets.only(left: 16, right: 16, top: 10, bottom: 16 + bottomInset),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Cross-Sell: sering dibeli bersama',
              style: GoogleFonts.poppins(
                  fontWeight: FontWeight.w800, fontSize: 16)),
          const SizedBox(height: 6),
          Text('Rekomendasi pasangan produk dari transaksi 30 hari terakhir.',
              style: GoogleFonts.poppins(
                  fontSize: 12,
                  color: Theme.of(context)
                      .colorScheme
          ...
          Flexible(
            child: FutureBuilder<List<Map<String, dynamic>>>(
                future: _pairsFuture,
                builder: (context, snapPairs) {
                  final pairs = snapPairs.data ?? const <Map<String, dynamic>>[];
                  if (snapPairs.connectionState == ConnectionState.waiting) {
                    return const Center(
                        child: Padding(
                            padding: EdgeInsets.symmetric(vertical: 18),
                            child: CircularProgressIndicator()));
                  ...
                  if (pairs.isEmpty) {
                    return Card(
                      child: Padding(
                        padding: const EdgeInsets.all(14),
                        child: Text('Belum ada data pasangan co-purchase.',
                            style: GoogleFonts.poppins(
                                fontSize: 12,
                                color: Theme.of(context)
                                    .colorScheme
                  ...
                  return FutureBuilder<List<Map<String, dynamic>>>(
                      ...
                      builder: (context, snapBundle) {
                        final bundles =
                            snapBundle.data ?? const <Map<String, dynamic>>[];
                        final map = <String, Map<String, dynamic>>{};
                        for (final b in bundles) {
                          final k =
                              '${b['productIdA']}-${b['productIdB']}';
                          map[k] = b;
                        }
                        return ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            final p = pairs[index];
                            final nameA = (p['nameA'] ?? '').toString();
                            final nameB = (p['nameB'] ?? '').toString();
                            final count =
                                (p['pairCount'] as num?)?.toInt() ?? 0;
                            final priceA =
                                (p['priceA'] as num?)?.toDouble() ?? 0.0;
                            final priceB =
                                (p['priceB'] as num?)?.toDouble() ?? 0.0;
                            final comboPrice = priceA + priceB;
                            final k =
                                '${p['productIdA']}-${p['productIdB']}';
                            final bundle = map[k];
                            final suggested =
                                (bundle?['suggestedBundlePrice'] as num?)
                                        ?.toDouble() ??
                                    0.0;
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 6),
                                    Text(
                                        'Sering dibeli bersama • $count transaksi',
                                        style: GoogleFonts.poppins(
                                            fontSize: 12,
                                            color: Theme.of(context)
                                                .colorScheme
                                    ...
                                    Row(
                                      children: [
                                        if ((p['stockA'] as num?) != null)
                                          Text('Stok A: ${(p['stockA'] as num).toInt()}',
                                              style: GoogleFonts.poppins(
                                                  fontSize: 12,
                                                  color: Theme.of(context)
                                                      .colorScheme
                                                      ...
                                                      .withOpacity(0.7))),
                                        const SizedBox(width: 12),
                                        if ((p['stockB'] as num?) != null)
                                          Text('Stok B: ${(p['stockB'] as num).toInt()}',
                                              style: GoogleFonts.poppins(
                                                  fontSize: 12,
                                                  color: Theme.of(context)
                                                      .colorScheme
                                    ...
                                    if (suggested > 0)
                                      Text(
                                          'Harga paket rekomendasi: ${fmt.format(suggested)} (normal ${fmt.format(comboPrice)})',
                                          style: GoogleFonts.poppins(
                                              fontSize: 12,
                                              color: Theme.of(context)
                                                  .colorScheme
...
class _UpsellSheet extends StatefulWidget {
  final Future<void> Function(String text) onShare;
  final Future<void> Function(String text) onWhatsApp;
  const _UpsellSheet({required this.onShare, required this.onWhatsApp});
  @override
  State<_UpsellSheet> createState() => _UpsellSheetState();
}

class _UpsellSheetState extends State<_UpsellSheet> {
  late final Future<List<Map<String, dynamic>>> _upsellFuture;
  ...
  void initState() {
    ...
    _upsellFuture = () async {
      final top = await DatabaseHelper.instance.getTopSellingProductsDetailed(
          limit: 1, start: start, end: end);
      String? pid = top.isNotEmpty ? (top.first['productId']?.toString()) : null;
      return AiService()
          .computeUpsellAddOns(forProductId: pid, start: start, end: end, limit: 20, addonPriceRatioMax: 0.4);
    }();
  }

  String _formatUpsellCaption({
    required String primaryName,
    required String addonName,
    required double addonPrice,
  }) {
    return AiService().generateUpsellCaption(
        primaryName: primaryName, addonName: addonName, addonPrice: addonPrice);
  }
        ...
        children: [
          Text('Upsell Add-On',
              style: GoogleFonts.poppins(
                  fontWeight: FontWeight.w800, fontSize: 16)),
          const SizedBox(height: 6),
          Text('Add-on berharga rendah yang sering dibeli bersama produk utama.',
              style: GoogleFonts.poppins(
                  fontSize: 12,
                  color: Theme.of(context)
                      .colorScheme
              ...
              builder: (context, snap) {
                ...
                if (items.isEmpty) {
                  return Card(
                    child: Padding(
                      padding: const EdgeInsets.all(14),
                      child: Text('Belum ada rekomendasi upsell add-on.',
                          style: GoogleFonts.poppins(
                              fontSize: 12,
                              color: Theme.of(context)
                                  .colorScheme
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    final u = items[index];
                    final primaryName = (u['primaryName'] ?? '').toString();
                    final addonName = (u['addonName'] ?? '').toString();
                    final addonPrice =
                        (u['addonPrice'] as num?)?.toDouble() ?? 0.0;
                    final pairCount =
                        (u['pairCount'] as num?)?.toInt() ?? 0;
                    return Card(
                      child: Padding(
                        padding: const EdgeInsets.all(14),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('$primaryName + $addonName',
                                style: GoogleFonts.poppins(
                                    fontWeight: FontWeight.w800)),
                            const SizedBox(height: 6),
                            Text(
                                'Sering dibeli bersama • $pairCount transaksi • Harga add-on: ${fmt.format(addonPrice)}',
                                style: GoogleFonts.poppins(
                                    fontSize: 12,
                                    color: Theme.of(context)
                                        .colorScheme
                            ...
                            Row(
                              children: [
                                Expanded(
                                  child: OutlinedButton(
                                    onPressed: () async {
                                      final text = _formatUpsellCaption(
                                          primaryName: primaryName,
                                          addonName: addonName,
                                          addonPrice: addonPrice);
                                      await widget.onShare(text);
                                    },
                                    child: const Text('Bagikan Add-On'),
                                  ),
                                ),
                                const SizedBox(width: 10),
                                Expanded(
                                ...
                                Expanded(
                                  child: FilledButton(
                                    onPressed: _openMarketing,
                                    child: const Text('Buat Diskon Add-On'),
                                  ),
                                ),
                              ],
                            ),
