...
class _MerchantOnboardingScreenState extends State<MerchantOnboardingScreen> {
  ...
  Future<void> _updateStoreProfileIfNeeded() async {
    ...
    try {
      ...
      final ownerName = (profile?['ownerName'] ?? profile?['owner_name'] ?? profile?['full_name'] ?? '').toString().trim();
      final lat = profile?['latitude']?.toString();
      final lng = profile?['longitude']?.toString();
      if (wa.isEmpty || address.isEmpty || ownerName.isEmpty) return;
  ...
  Widget buildConfirmStep() {
    ...
    final priceText =
        priceController.text.isEmpty ? '${MerchantConfig.defaultCurrencySymbol} 0' : '${MerchantConfig.defaultCurrencySymbol} ${priceController.text}';
