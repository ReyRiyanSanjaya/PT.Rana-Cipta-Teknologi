...
class _FlashSalesScreenState extends State<FlashSalesScreen> {
  ...
  Future<void> _createSale() async {
    if (_titleController.text.isEmpty || _startAt == null || _endAt == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Judul dan periode wajib diisi'),
    ...
    if (!(_startAt!.isBefore(_endAt!))) {
      final colorScheme = Theme.of(context).colorScheme;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Tanggal mulai harus sebelum tanggal berakhir'),
        backgroundColor: colorScheme.primary,
      ));
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        final colorScheme = Theme.of(context).colorScheme;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text('Flash Sale berhasil dibuat'),
          backgroundColor: colorScheme.primary,
        ));
      }
    } catch (e) {
  ...
  Future<void> _addItem() async {
    if (_selectedSaleId.isEmpty) {
      final colorScheme = Theme.of(context).colorScheme;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Pilih Flash Sale terlebih dahulu'),
        backgroundColor: colorScheme.primary,
      ));
      return;
    }
    ...
    if (salePrice == null || salePrice <= 0) {
      final colorScheme = Theme.of(context).colorScheme;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Harga sale harus valid dan > 0'),
        backgroundColor: colorScheme.primary,
      ));
      return;
    }
  ...
  void _showProductPicker() {
    if (_products.isEmpty) {
      final colorScheme = Theme.of(context).colorScheme;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Tidak ada produk tersedia'),
        backgroundColor: colorScheme.primary,
      ));
      return;
    }
  ...
  Map<String, dynamic>? _getSelectedSale() {
    try {
      return _sales.firstWhere((s) => (s['id'] ?? '').toString() == _selectedSaleId);
    } catch (_) {
      return null;
    }
  }
  Future<void> _shareFlashSaleCaption() async {
    if (_selectedProductId == null || _selectedProductId!.isEmpty) {
      final colorScheme = Theme.of(context).colorScheme;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Pilih produk terlebih dahulu'),
    ...
    final original = (product['sellingPrice'] as num?)?.toDouble() ?? 0.0;
    final salePrice = double.tryParse(_salePriceController.text) ?? 0.0;
    final sale = _getSelectedSale();
    DateTime? endsAt;
    if (sale != null) {
      endsAt = DateTime.tryParse((sale['endAt'] ?? '').toString());
    }
    final caption = AiService().generateSmartPriceCaption(
      name: name,
      originalPrice: original,
      newPrice: salePrice > 0 ? salePrice : original,
      promoEndsAt: endsAt,
      category: null,
    );
    await Share.share(caption);
  }
  Future<void> _whatsappFlashSaleCaption() async {
    if (_selectedProductId == null || _selectedProductId!.isEmpty) {
      final colorScheme = Theme.of(context).colorScheme;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Pilih produk terlebih dahulu'),
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: _shareFlashSaleCaption,
                            icon: const Icon(Icons.share),
                            label: const Text('Bagikan Caption'),
                          ),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: _whatsappFlashSaleCaption,
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: colorScheme.primaryContainer
                                .withOpacity(isDarkTheme ? 0.16 : 0.18),
                                              ...
                                              Wrap(
                                                ...
                                                children: [
                                                  Container(
                                                    ...
                                                    child: Text(
                                                      '${MerchantConfig.defaultCurrencySymbol} ${NumberFormat('#,###').format((it['salePrice'] ?? 0).round())}',
                                                      style:
                                                          GoogleFonts.poppins(
                                                        fontSize: 12,
                                                        fontWeight:
