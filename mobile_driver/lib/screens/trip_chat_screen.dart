...
class _TripChatScreenState extends State<TripChatScreen> {
  ...
  static const List<String> _templates = [
    ...
    'Bisa tolong keluar ke depan?',
    'Pesanan sudah saya ambil',
    'Terima kasih 🙏',
  ];
  ...
  void dispose() {
    _chatSub?.cancel();
    if (_roomId != null) SocketService().leaveChatRoom(_roomId!);
    _msgCtrl.dispose();
    _scrollCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _initChat() async {
    try {
      ...
      final result = await DriverApiService().getTripChatRoom(widget.requestId);
      ...
      if (_roomId != null) {
        ...
        final messages = await DriverApiService().getChatMessages(_roomId!);
        if (mounted) {
          setState(() {
  ...
  Future<void> _sendMessage(String content) async {
    ...
    await DriverApiService().sendChatMessage(_roomId!, content);
  }
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            ...
            Text('Pelanggan', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
          ],
        ),
